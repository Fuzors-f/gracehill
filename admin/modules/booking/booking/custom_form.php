<script>
    $(function(){
        ...
        // fendy
        // for edit if exist booking room
        // let count_row_bookroom = $("#table_booking_room tr").length;
       /* if($("#booking_room_amount_0_0").val() != null || $("#booking_room_amount_0_0").val() != 0 ){
            for(let i =0; i< count_row_bookroom; i++){
                room_rate.push({
                    'id' : i,
                    'price' : $("#booking_room_amount_0_"+i).val()
                });
            }
        } */
		...
		/*function updateNights(){
			...
			//fendy change room booking value 
			for(let i=0; i< room_rate.length; i++){
			    	$('#booking_room_amount_0_'+room_rate[i].id).val(room_rate[i].price * parseInt(nnights));
			    	//$('#booking_room_amount_0_'+room_rate[i].id).val(room_rate[i].price);
			}
		...
		$('#table_booking_room').on('change', '[name^="booking_room_id_room_0"]', function(){
			...
			// fendy 
			var total = parseFloat($("#booking_total_0_0").val());
			if(room_id > 0){
				$.ajax({
                    type: 'POST',
                    url: 'get_room.php',
                    ...
                    success: function(data){
						...
						if(data.id !== undefined){
							...
							room_rate.push({
								...
								$('#booking_room_amount_0_'+id).val(data.price * parseInt(nnights_input.val()));
								//	$('#booking_room_amount_0_'+id).val(data.price );
								// fendy set price
								total = total + (data.price * parseInt(nnights_input.val()));
								$("#booking_total_0_0").val(total);
						}
                    }
                });
			}
