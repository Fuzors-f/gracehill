...
if($db !== false){
    ...
    $query_search= "";
    $tmp = null;
    // if(MODULE == "booking_activity"){
    //     $tmp =$cols;
    //     //array("firstname", "lastname","status")
    //     $query_search = db_getRequestSelect($db, 'pm_booking', array("firstname", "lastname","status") , $q_search, $condition, $order.' '.$sort);
    // }else{
                                            ...
                                            <tbody>
                                                ...
                                                if($result !== false){
                                                    ...
                                                    foreach($result as $i => $row){
                                                        ...
                                                        // if(MODULE == "booking_activity"){
                                                        //     $data_activity = $db->query("select * from pm_booking_activity where id_booking = ". $id);
                                                        //    // $data_activity = $data_activity ->fetch();
                                                        //     var_dump($data_activity);
                                                        //    foreach($data_activity as $ix => $row_activity){
                                                        //        $row['id_activity'] = $row_activity["id"];
                                                        //        $row['title_activity'] = $row_activity["title"];
                                                        //        $row['children_activity'] = $row_activity["children"];
                                                        //        $row['adult_activity'] = $row_activity["adults"];
                                                        //        $row['amount_activity'] = $row_activity["amount"];
                                                        //        $row['duration_activity'] = $row_activity["duration"];
                                                        //        $row['date_activity'] = (is_null($row_activity['date'])) ? '-' : strftime(DATE_FORMAT.' '.TIME_FORMAT, $row_activity['date']);
                                                        //    }
                                                        // }
                                                       $status = "";
                                                        if(isset($row['status'])){
                                                        ...
                                                        <tr id="item_<?php echo $id ?>">
                                                            ...
                                                            <td class="text-center">
                                                                ...
                                                                if(MODULE == "booking"){ 
                                                                        $data_status = null;
                                                                        if($status == 1) $data_status = "PENDING";
                                                                        else if($status == 2) $data_status = "CANCELLED";
                                                                        else if($status == 3) $data_status = "PAYMENT_REJECTED";
                                                                        else $data_status = "CONFIRMED";
                                                                    
                                                                    ?>
                                                                    <a data-html="true" data-container="body" class="tips ajax-popup-link <?php $data_status?>" href="popup-details.php" title="Get Status Booking" data-params="id=<?php echo $id ?>"><i class="fas fa-list"></i></a>
                                                                    <?php
                                                                }
                                                                if(in_array('delete', $permissions) || in_array('all', $permissions)){ ?>
