...
function db_getRequestSelect($db, $table, $cols, $q, $condition_sup = '', $order = '', $limit = '', $offset = '')
{
    ...
    $nb_cols = count($cols);
    $query = "";
    if($table == "pm_booking_activity"){
        $query = 'select book.id as id,  book.lastname as lastname, book.firstname as firstname, book.status as status, book.add_date as add_date, book.edit_date as edit_date, book_activity.adults as adults, book_activity.children as children, book_activity.duration as duration, book_activity.title as title, book_activity.amount as amount, book_activity.date as date from pm_booking_activity book_activity, pm_booking book';
    }
    else if ($table == "pm_booking"){
        $query = ' SELECT *, (total - down_payment) as remain from pm_booking';
    }
    ...
    foreach($cols as $j => $col){
        ...
        foreach($arr_colname as $i => $str_colname){
            ...
            $col_type = db_column_type($db, $table, $str_colname);
               ...
               if($table == "pm_booking_activity"){
                if($str_colname == "amount") {
                    $str_colname = 'book_activity.amount';
                } else if($str_colname == "children"){
                    $str_colname = 'book_activity.children';
                
                } else if($str_colname == "adults"){
                    $str_colname = 'book_activity.adults';
                
                } else if($str_colname == "status"){
                    $str_colname = 'book.status';
                }
            }

           if($table == "pm_booking"){
               if($str_colname == "remain"){
                   $cek = false;
               }
           }
           if($cek == true){
    ...
    if($table == "pm_booking_activity"){
        $query .= ($condition != '') ? ' WHERE book_activity.id_booking = book.id and ('.$condition : '' ;
        
    } else if($table == "pm_booking"){
        $query .= ($condition != '') ? ' WHERE from_date is not null and to_date is not null and ('.$condition : '' ;
    }
    else{
